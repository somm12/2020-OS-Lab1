...
void rr_4(process arr[], Queue* pq, int time, int size) {
	...
	for (int i = 0; i < time; i++) {					// total_time 만큼 횟수 반복
		...
		if (signal == -1) {		// 프로세스의 실행이 막 끝났을 때 또는 실행 중인 프로세스가 없을 때
			...
			if (time_slice == 0&&QIsEmpty(pq)==0)running[0] = QPeek(pq);// 첫 프로세스의 time slice가 끝날 때까지 값을 받아오지 않고 마지막 프로세스가 수행을 마치면 더이상 값을 불러오지 않는다.
...
/**************************************************** MLFQ Implementation (Q=4) ****************************************************************/


void mlfq_4(process arr[], int time, int size) {
	Queue P1;
	Queue P2;
	Queue P3;							// 우선순위를 정해주기 위한 큐 4개
	Queue P4;							// P1이 우선순위가 제일 높으며, P4가 우선순위가 제일 낮다
